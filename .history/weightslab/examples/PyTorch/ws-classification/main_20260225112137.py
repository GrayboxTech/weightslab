...
def _mnist_raw_complete(raw_dir):
    """Return True if all MNIST raw files (or their .gz variants) are present."""
    required_basenames = [
    ...
    for basename in required_basenames:
        uncompressed = os.path.join(raw_dir, basename)
        compressed = f"{uncompressed}.gz"
        if not (os.path.exists(uncompressed) or os.path.exists(compressed)):
            return False
...
if __name__ == "__main__":
    ...
    if parameters.get("data_dir"):
        ...
        # Case 1: data_dir ends in .../MNIST/raw
        if _mnist_raw_complete(user_path):
            data_root = os.path.dirname(os.path.dirname(user_path))
            should_download = False
            print(f"Using existing data from {user_path}")
        else:
            # Case 2: data_dir is torchvision root (.../<root>) and files are in .../<root>/MNIST/raw
            check_child = os.path.join(user_path, "MNIST", "raw")
            if _mnist_raw_complete(check_child):
                data_root = user_path
                should_download = False
                print(f"Using existing data found in {check_child}")
            else:
                # Case 3: data_dir is .../MNIST and files are in .../MNIST/raw
                check_sibling = os.path.join(user_path, "raw")
                if _mnist_raw_complete(check_sibling):
                    data_root = os.path.dirname(user_path)
                    should_download = False
                    print(f"Using existing data found in {check_sibling}")
                else:
                    # Not found or incomplete, download to the specified directory (acting as root)
                    data_root = user_path
                    should_download = True
                    print(f"Data not found or incomplete, will download to {data_root}")
    else:
