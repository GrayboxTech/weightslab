...
class DataService:
    ...
    def __init__(self, ctx):
        ...
        dt_pull = time.time() - t0
        if dt_pull > 0.5:
            logger.info(f"Initial datasets pull took {dt_pull:.2f}s")
        ...
        # Run heavy startup tasks in background to avoid blocking training start
        def _background_startup():
            try:
                t_aspect = time.time()
                self._deduce_and_set_aspect_ratios()
                dt_aspect = time.time() - t_aspect
                
                if self._compute_natural_sort:
                    t_sort = time.time()
                    self._compute_natural_sort_stats()
                    dt_sort = time.time() - t_sort
                    logger.info(f"Background startup: aspect ratios ({dt_aspect:.2f}s), natural sort ({dt_sort:.2f}s)")
                elif dt_aspect > 1.0:
                    logger.info(f"Background startup: aspect ratios ({dt_aspect:.2f}s)")
            except Exception as e:
                logger.warning(f"Error in background DataService startup: {e}")

        self._data_executor.submit(_background_startup)
    ...
    def _process_sample_row(self, args):
        ...
        from PIL import Image  # Force local binding to fix UnboundLocalError
        ...
        try:
            ...
            if (is_label_empty or (force_reload and not isinstance(label, np.ndarray))) and dataset:
                try:
                    label = load_label(dataset, sample_id)
                except Exception as e:
                    logger.debug(f"Failed to load label for sample {sample_id}: {e}")
                    label = None
            ...
            if task_type != "classification":
                ...
                if label_arr is not None:
                    ...
                    try:
                        import cv2
                        if label_arr.ndim >= 2 and label_arr.size > 10000:
                             h, w = label_arr.shape[:2]
                             if max(h, w) > 256:
                                 scale = 256.0 / max(h, w)
                                 label_arr = cv2.resize(np.asarray(label_arr).astype(np.float32), (0,0), fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST)
                    except Exception:
                        pass
                    ...
                    data_stats.append(
                        create_data_stat(
                            name='label', # WeightsLab UI expects 'label' for Ground Truth segmentation masks
                            stat_type='array',
                ...
                if isinstance(label, dict):
                    ...
                    if lidar_config and 'boxes' in label:
                        logger.debug(f"[DataService] Rendering BEV mask. Boxes: {len(label['boxes'])}")
                        ...
                        try:
                            import cv2
                            if mask.size > 65536:
                                 scale = 256.0 / max(mask.shape)
                                 mask = cv2.resize(mask.astype(np.float32), (0,0), fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST)
                        except Exception:
                            pass

                        data_stats.append(
                            create_data_stat(
                                name='target_mask', # Use target_mask convention
                                stat_type='array',
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                import cv2
                                if label_arr.size > 65536:
                                     scale = 256.0 / max(label_arr.shape)
                                     label_arr = cv2.resize(label_arr.astype(np.float32), (0,0), fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST)
                            except Exception:
                                pass
                            data_stats.append(
                                create_data_stat(
            ...
            if task_type != "classification" and pred is not None:
                ...
                # Note: lidar_config is resolved in the label section above if task_type requires it.
                # If we are here, we might need to resolve it again if it wasn't.
                lidar_config = {}
                if isinstance(pred, dict) and dataset:
                    curr_ds = dataset
                    while True:
                         if hasattr(curr_ds, "viz_config"):
                             lidar_config = curr_ds.viz_config
                ...
                if isinstance(pred, dict) and lidar_config and 'boxes' in pred:
                     ...
                     try:
                         ...
                         try:
                             if mask.size > 65536:
                                  h, w = mask.shape[:2]
                                  img_temp = Image.fromarray(mask.astype(np.uint8))
                                  scale = 256.0 / max(h, w)
                                  resample_mode = getattr(getattr(Image, "Resampling", Image), "NEAREST", Image.NEAREST)
                                  img_temp = img_temp.resize((int(w * scale), int(h * scale)), resample_mode)
                    ...
                    try:
                        ...
                        try:
                            import cv2
                            if pred_arr.size > 65536:
                                 scale = 256.0 / max(pred_arr.shape)
                                 pred_arr = cv2.resize(pred_arr.astype(np.float32), (0,0), fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST)
                        except Exception:
                            pass
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                import cv2
                                if pred_.size > 65536:
                                     scale = 256.0 / max(pred_.shape)
                                     pred_ = cv2.resize(pred_.astype(np.float32), (0,0), fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST)
                            except Exception:
                                pass
                            data_stats.append(
                                create_data_stat(
            ...
            if request.include_raw_data:
                ...
                if input_arr is not None and isinstance(input_arr, np.ndarray):
                    # We have a pre-rendered image in the row
                    np_img = input_arr
                    is_volumetric = False
                    original_shape = list(input_arr.shape)
                    # Convert to PIL for thumbnail processing
                    if input_arr.ndim == 2:
                        middle_pil = Image.fromarray(input_arr, mode="L")
                    else:
                        # Ensure it's (H, W, 3) for RGB
                        if input_arr.ndim == 3 and input_arr.shape[-1] == 3:
                            middle_pil = Image.fromarray(input_arr, mode="RGB")
                        else:
                ...
                if middle_pil is not None:
                    ...
                    if target_width != original_size[0] or target_height != original_size[1]:
                        # Use faster resampling for thumbnails (BILINEAR), high quality for full res (LANCZOS)
                        method_name = "LANCZOS" if is_full_resolution else "BILINEAR"
                        resample_mode = getattr(getattr(Image, "Resampling", Image), method_name, Image.NEAREST)
                        middle_pil = middle_pil.resize((target_width, target_height), resample_mode)
    ...
    def _process_get_data_samples(self, request, context):
        ...
        try:
            ...
            logger.debug("Processing %s samples tasks at %s", len(tasks), datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
            data_records = self._data_executor.map(self._process_sample_row, tasks, timeout=300)
