...
HyperParam = namedtuple(
    "HyperParam",
    ["data_type", "value", "default_value", "fct2call", "related_functions"]
)
...
class Experiment:
    ...
    def __init__(
        ...
        self.global_hyper_parameters: Dict[str, HyperParam] = {}
    ...
    # Or maybe not.
    def watch_or_edit(self, obj: Callable) -> None:
        """Generate setters getters"""

        def getter(instance: 'Experiment'):
            """Dynamically generated getter for callable object"""
            with instance.lock:
                return obj

        # Define the Setter function template
        def setter(instance: 'Experiment', obj: Callable):
            """Dynamically generated setter for callable object"""
            obj_name = obj.__name__
            with instance.lock:
                setattr(instance, obj_name, obj)

        # Assign the functions as new methods to the instance using setattr()
        setattr(self, f"get_{obj.__name__}", getter)
        setattr(self, f"set_{obj.__name__}", setter)

    def watch(self, obj: Callable, flag="None") -> None:
        """Generate setters getters"""
