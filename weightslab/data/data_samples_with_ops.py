...
class DataSampleTrackingWrapper(Dataset):
    ...
    @property
    def task_type(self):
        """Expose task_type from the wrapped dataset if available."""
        # Try self.wrapped_dataset (e.g. Subset)
        # Then try self.wrapped_dataset.dataset (the original dataset)
        task_type = getattr(self.wrapped_dataset, "task_type", None)
        if task_type is None and hasattr(self.wrapped_dataset, "dataset"):
            task_type = getattr(self.wrapped_dataset.dataset, "task_type", None)
        return task_type
