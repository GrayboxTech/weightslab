...
def load_raw_image(dataset, index, slice_idx: int = None, rank: int = 0) -> Image.Image:
    ...
    # Get dataset wrapper if exists
    wrapped = getattr(dataset, "wrapped_dataset", dataset)
    # Check for Subset (recursive)
    while (hasattr(wrapped, "dataset") and hasattr(wrapped, "indices")):
    ...
    if hasattr(wrapped, "images") and isinstance(wrapped.images, (list, tuple, np.ndarray)):
        ...
        if isinstance(img_path, str) and img_path.lower().endswith(('.off', '.bin', '.pcd', '.ply', '.npy')):
             from weightslab.trainer.services.lidar_utils import render_lidar, load_point_cloud_data
             points = load_point_cloud_data(img_path)
             if points is not None:
                  return render_lidar(points, lidar_config, file_path=img_path)
        img = Image.open(img_path)
        return img.convert("RGB")
    elif hasattr(wrapped, "files") and isinstance(wrapped.files, (list, tuple)):
        ...
        # Handle Point Cloud auto-detection in nested data
        if _is_point_cloud(np_img):
            from weightslab.trainer.services.lidar_utils import render_lidar
            points = to_numpy_safe(np_img)
            if points is not None:
