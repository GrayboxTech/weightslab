...
class CheckpointManager:
    ...
    def __init__(self, root_log_dir: str = 'root_experiment', load_model: bool = True, load_config: bool = True, load_data: bool = True, load_loggers: bool = True):
        ...
        # Automatically resume latest state if available
        # This will call load_state which handles loading model, config, weights, data, and loggers
        t_boot = time.time()
        
        target = self.current_exp_hash or self.get_latest_hash()
        
        if target:
            # Bootstrap existing state
            self._bootstrap_latest_state(load_model=load_model, load_config=load_config, load_data=load_data, load_loggers=load_loggers)
        else:
            # No specific experiment yet; load initial global logger snapshots if any
            t_log = time.time()
            self._load_all_logger_snapshots()
            dt_log = time.time() - t_log
            if dt_log > 1.0:
                 logger.info(f"Logger snapshots loaded in {dt_log:.2f}s")

        dt_boot = time.time() - t_boot
        if dt_boot > 1.0:
            logger.info(f"Bootstrap latest state finished in {dt_boot:.2f}s")
            ...
            for path in chunk_paths:
                ...
                try:
                    with open(path, "rb") as f:
                        # Stream decompression to save memory for large payloads
                        ...
                        for line in text_stream:
                            ...
                            try:
                                ...
                                if lname:
                                    # Merge if logger already seen in previous chunks (unlikely but possible)
                                    if lname in snapshot["loggers"] and isinstance(payload, dict) and isinstance(snapshot["loggers"][lname], dict):
                                         snapshot["loggers"][lname].update(payload)
                                    else:
    ...
    def _bootstrap_latest_state(self, load_model: bool = True, load_config: bool = True, load_data: bool = True, load_loggers: bool = True):
        ...
        if not target:
            return
        try:
            self.load_state(target, load_model=load_model, load_config=load_config, load_data=load_data, load_loggers=load_loggers)
        except Exception as e:
    ...
    def update_experiment_hash(
        ...
        has_changed, changed_components = self.hash_generator.has_changed(
            ...
            model_init_step=self._model_init_step,
            force=force,
            _last_time_loaded=self._last_time_loaded
        )
    ...
    def _load_all_logger_snapshots(self):
        ...
        manifest_path = self.loggers_dir / self.LOGGER_SNAPSHOT_MANIFEST_FILE
        has_snapshot = manifest_path.exists()
        
        if not has_snapshot:
             has_snapshot = (self.loggers_dir / "loggers.json").exists()
             if not has_snapshot:
                 # Only glob if manifest and old json are missing
                 has_snapshot = any(self.loggers_dir.glob(f"{self.LOGGER_SNAPSHOT_CHUNK_PREFIX}*{self.LOGGER_SNAPSHOT_CHUNK_SUFFIX}"))
    ...
    def load_logger_snapshot(self) -> bool:
        ...
        try:
            ...
            registered_loggers = set(ledgers.list_loggers())
            
            for lname, payload in loggers_payload.items():
                try:
                    lg = ledgers.get_logger(lname) if lname in registered_loggers else None
            ...
            dt = time.time() - t0
            if dt > 1.0:
                logger.info(f"Loaded logger snapshot for {len(loggers_payload)} loggers in {dt:.2f}s")
            return True
        except Exception as e:
