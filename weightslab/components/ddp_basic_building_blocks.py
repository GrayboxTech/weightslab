...
# ① AGGREGATE_UP
# ---------------------------------------------------------------------------
def aggregate_up(combine):
    """Decorator factory. `combine(parts) -> (args, kwargs)` merges the list of
    per-rank (args, kwargs). rank 0 runs the body once on the merged params;
    children send and return None. Single-process = passthrough."""
    def deco(fn):
        @functools.wraps(fn)
        def wrapped(*args, **kwargs):
            ...
            r, _ = ddp_info()
            bucket = _gather((args, kwargs), what=f"↑{fn.__name__}")
            if r == 0:
                margs, mkwargs = combine(bucket)
                return fn(*margs, **mkwargs)
            return None
        return wrapped
    return deco
...
# ② REPLICATE_DOWN
# ---------------------------------------------------------------------------
def replicate_down(fn):
    """Decorator: rank 0's call params are broadcast; every rank runs the body with
    them (identical local mutation). Single-process = passthrough."""
    @functools.wraps(fn)
    def wrapped(*args, **kwargs):
        if not _active():
            return fn(*args, **kwargs)
        args, kwargs = _broadcast([(args, kwargs)], what=f"↓{fn.__name__}")
        return fn(*args, **kwargs)
    return wrapped


# ---------------------------------------------------------------------------
# ③ RECONCILE — single state (per-consumption-point / ad-hoc, 1 broadcast)
# ---------------------------------------------------------------------------
def reconcile_down(name, snapshot, apply):
    """Single-state reconcile hook (1 broadcast/call). For per-consumption-point
    use (e.g. reconcile lr right inside optimizer.step). For the per-step anchor
    reconcile of MANY states, prefer register_consistent_state + reconcile_all
    (one bundled broadcast for all of them)."""
    def hook():
        if not _active():
            return None
        r, _ = ddp_info()
        snap = _broadcast([snapshot() if r == 0 else None], what=f"↓{name}")
        if r != 0:
            apply(snap)
        return snap
    return hook
...
# ---------------------------------------------------------------------------
# combine helpers for aggregate_up
# ---------------------------------------------------------------------------
def _concat(pieces):
    """Concatenate per-rank lists / 1-D tensors / arrays into one (order = rank)."""
    if not pieces:
        return pieces
    try:
        import torch
        if isinstance(pieces[0], torch.Tensor):
            return torch.cat(list(pieces), dim=0)
    except Exception:
        pass
    out = []
    for p in pieces:
...
def combine_rank0(parts):
    """Trivial combine: use rank 0's own params (when already identical across ranks)."""
    return parts[0]


def make_concat_combine(arg_positions=(), list_kwargs=(), dict_kwargs=()):
    """Build a `combine` that concatenates per-sample args across ranks and takes
    rank 0's value for everything else (assumed identical).

    arg_positions: indices into *args that are per-sample lists / 1-D tensors.
    list_kwargs:   kwarg names that are per-sample lists / 1-D tensors.
    dict_kwargs:   kwarg names that are per-sample dicts to union.
    """
    def combine(parts):
        base_args, base_kwargs = parts[0]
        margs = list(base_args)
        for pos in arg_positions:
            margs[pos] = _concat([p[0][pos] for p in parts])
        mkwargs = dict(base_kwargs)
        for k in list_kwargs:
            mkwargs[k] = _concat([p[1][k] for p in parts])
        for k in dict_kwargs:
            merged = {}
            for p in parts:
                merged.update(p[1].get(k, {}) or {})
            mkwargs[k] = merged
        return tuple(margs), mkwargs
    return combine
