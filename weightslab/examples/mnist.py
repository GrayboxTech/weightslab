...
if __name__ == '__main__':
    ...
    def training_thread_callback():
        for train_step, (inputs, ids, labels) in enumerate(tqdm.tqdm(train_loader, desc='Training..')):
            ...
            with wl_exp.training_guard:
                ...
                train_losses_batch = torch.cat([losses_batch_bin[..., None], losses_batch_mlt[..., None]], axis=1)

                # Update dataset statistics
                wl_exp.update_data_statistics(
                    model_age,
                    ids,
                    train_losses_batch.mean(1),
                    preds
    ...
    # t = Thread(target=training_thread_callback, daemon=True)
    # t.start()

    serve(wl_exp, threading=True)
    training_thread_callback()
