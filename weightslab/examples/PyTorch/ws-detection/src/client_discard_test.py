...
N_DISCARD = 15
TRAIN_STEPS_TO_WAIT = 300
POLL_INTERVAL = 5

# WL flattens "/" → "__SLASH__" in signal column names
SIG_BOX = "signals__SLASH____SLASH__train__SLASH__box_per_sample"
SIG_CLS = "signals__SLASH____SLASH__train__SLASH__cls_per_sample"
SIG_DFL = "signals__SLASH____SLASH__train__SLASH__dfl_per_sample"
SIG_IOU = "signals__SLASH____SLASH__val__SLASH__iou_per_sample"
...
def fetch_records(stub, origin, signals):
    req = pb2.DataSamplesRequest(
        ...
        stats_to_retrieve=["last_seen", "discarded", "origin"] + signals,
    )
...
def pick_top_by(records, signals, sort_key, n):
    """Return n sample_ids with the highest value at sort_key, requiring
    that all `signals` are present (so we have something to compare later)."""
    rows = []
    for sid, rec in records.items():
        sigs = {s: _as_float(_get_stat(rec, s)) for s in signals}
        if any(v is None for v in sigs.values()):
            continue
        if sigs[sort_key] is None:
            continue
        rows.append((sid, sigs[sort_key]))
...
def verify_frozen(label, victims, pre, post, signals):
    ...
    for v in victims:
        ...
        changed = []
        for s in signals:
            if pre_sigs[s] is None or post_sigs[s] is None:
                continue
            if pre_sigs[s] != post_sigs[s]:
                changed.append((s, pre_sigs[s], post_sigs[s]))
        if changed:
...
def main():
    ...
    while True:
        ...
        print(f"[client] max_train_last_seen={max_train} "
              ...
              flush=True)
        if (max_train >= 200 and train_with_box >= N_DISCARD * 3
    ...
    pre_train = {v: snapshot_sample(train_recs[v], TRAIN_SIGNALS) for v in train_victims}
    pre_val = {v: snapshot_sample(val_recs[v], VAL_SIGNALS) for v in val_victims}
    ...
    # ── wait for ≥ TRAIN_STEPS_TO_WAIT post-discard ────────────────
    target = pre_max + TRAIN_STEPS_TO_WAIT
    ...
    while True:
        try:
            ...
            time.sleep(POLL_INTERVAL); continue
        cur = max_last_seen(train_recs)
        print(f"[client]   max_train_last_seen={cur} (target {target})", flush=True)
        if cur >= target:
            break
        time.sleep(POLL_INTERVAL)
