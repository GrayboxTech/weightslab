...
from utils.data_av2 import Av2LidarDetectionDataset
...
if __name__ == "__main__":
    ...
    # Argoverse 2 (Sensor) — train/val live in separate folders on disk, so it
    # has its own builder rather than the KITTI/synthetic internal split.
    av2_cfg = data_cfg.get("argoverse2", {}) or {}
    ...
    def _build_datasets(src):
        if src == "argoverse2":
            common = dict(
                ...
                sweep_stride=int(av2_cfg.get("sweep_stride", 1)),
                min_interior_pts=int(av2_cfg.get("min_interior_pts", 1)),
                thumbnail_projection=data_cfg.get("thumbnail_projection", "bev"),
            )
            train = Av2LidarDetectionDataset(
                av2_cfg.get("train_dir", os.path.join(data_root, "sensor", "train")),
                split="train", max_logs=av2_cfg.get("train_max_logs"),
                max_samples=train_cfg.get("max_samples", None), **common)
            val = Av2LidarDetectionDataset(
                av2_cfg.get("val_dir", os.path.join(data_root, "sensor", "val")),
                split="val", max_logs=av2_cfg.get("val_max_logs"),
                max_samples=test_cfg.get("max_samples", None), **common)
            return train, val
        common = dict(
    ...
    # One signal set shared by train + eval, so each sample has a single
    # continuous loss trajectory (WL tags the split; the dashboard splits it).
    def _make_det_signals(weights=None) -> dict:
        return {
            "loss": wl.watch_or_edit(
                PerSampleDetection3DLoss(num_classes, grid_size, pc_range, weights=weights),
                flag="loss",
                name="loss/sample", per_sample=True, log=True,
            ),
            "iou_sample": wl.watch_or_edit(
                PerSampleBevIoU(num_classes, grid_size, pc_range), flag="metric",
                name="iou/sample", per_sample=True, log=True,
            ),
            "iou_instance": wl.watch_or_edit(
                PerInstanceBevIoU(num_classes, grid_size, pc_range), flag="metric",
                name="iou/instance", per_instance=True, log=True,
            ),
        }
    ...
    train_sig = test_sig = _make_det_signals(weights=weights)
    ...
    # Per-sample signals are unified (one `loss/sample` column); these aggregate
    # curves keep train vs test separate on the dashboard.
    def _plot_scalar(name, value, step):
        if value is None:
            return
        lg = get_logger()
        if lg is None or not hasattr(lg, "add_scalars"):
            return
        lg.add_scalars(name, {name: float(value)}, global_step=int(step),
    ...
    for train_step in train_range:
        ...
        _plot_scalar("train_loss", train_loss, age)
        ...
        if age == 0 or age % eval_full_to_train_steps_ratio == 0:
            ...
            _plot_scalar("test_loss", test_loss, age)
            _plot_scalar("test_bev_iou", test_metric, age)
